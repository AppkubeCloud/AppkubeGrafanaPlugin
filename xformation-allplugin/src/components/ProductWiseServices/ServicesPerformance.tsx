                  ...
                  return (
                    <div className='tbody'>
                      <div className='td' style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}><span style={{ paddingLeft: '45px' }}>
                        <Link to={`${PLUGIN_BASE_URL}/plugins/asset-manager/department-wise-products/storage-details?accountId=${accountId}&cloudName=${cloudName}`} onClick={(e: any) => this.onClickDirectService(e, service)}>{service.name}</Link>
                      </span></div>
                      <div className='td'>
    ...
    tagList.forEach((tag: any, i: any) => {
      ...
      tag.serviceList && tag.serviceList.forEach((service: any, i: any) => {
        if (service.hostingType === this.props.hostingType) {
          servicesJSX.push(
            <div className='tbody'>
              <div className='td' style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}><span style={{ paddingLeft: '0px' }}><Link to={`${PLUGIN_BASE_URL}/plugins/asset-manager/department-wise-products/storage-details?accountId=${567373484}`} onClick={(e: any) => this.onClickDirectService(e, service)}>{service.name}</Link></span></div>
              <div className='td'>
                <div className={`progress-circle ${this.getPerformanceClass(service.performance.score)}`} >
                  <i className='fa fa-check-circle'></i>
                </div>
